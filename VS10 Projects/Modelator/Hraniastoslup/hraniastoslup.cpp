...
int main()
{
	...
	cout << "ilosc blokow: ";
	cout << "pomijam - tymczasowo niewazne!\n";
	int total = 0;


	cout << "nazwa pola: ";
	cin >> nazwa_pola;
	cout << "ile lacznie obiektow? : ";
	cin >> n_objects;
	...
	for( int i = 0; i < 6; i++ )
	{
		if( sciany[i] == '_' )
			continue;
		a++;
		cout << "id tekstury - " << sciany[i] << ": ";
		cin >> id_tex[i];
	}
	plik << "\nNUMPOLLIES " << a*2*n_objects << "\n\n";
	sciany += "-------"; // na wszelki wypadek, w ten sposob string sciany ma przynajmniej 6 znakow
	cout << "przesuniecie w x, y, z co ture: ";
	...
	for( int j = 0; j < n_objects; j++ )
	{
		...
		for( int i = 0; i < 6; i++ )
		{
			...
			wpisz_pole( pkt, u, v, &plik, id_tex[i] );
		}
	}
